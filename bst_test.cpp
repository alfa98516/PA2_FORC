#include <cassert>
#include <iostream>
#include "self_balancing_bst.hpp"

using std::cout;
using std::cin;
using std::ws;

LLRB<int, int> trees[1000];
bstNode<int, int>* cursors[1000];
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (op == 'a') {
            ...
            int key;
            cin >> key;
            cursors[instance] = trees[instance].lowerBound(key);
        }
        else if (op == 'u') {
            int key;
            cin >> key;
            cursors[instance] = trees[instance].upperBound(key);
        }
        else if (op == 'i') {
            int key, value;
            cin >> key >> value;
            ...
            int k;
            cin >> k;
            cursors[instance] = trees[instance].kElement(k);
        }
        else if (op == 'g') {
            // or other sentinel value
            if (cursors[instance] == NULL) {
